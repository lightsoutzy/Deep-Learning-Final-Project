...
def augment_dataset(img, label, count, file, seed=42):
    ...
    for idx in range(count):
        ...
        new_img = tf.image.stateless_random_hue(image=img, max_delta=0.05, seed=(seed, seed))
...
# # first copy out test dataset
# for d in label_names:
#     num_files = num_items[d]
#     count = 0
#     for filename in os.listdir(os.path.join(directory, d)): 
#         file = os.path.join(directory, d, filename)
#         if count % 3 == 0:
#             print("test: ", file)
#             os.rename(file, file.replace("tea sickness dataset", "tea_sickness_dataset_test"))
#         count += 1

# total_wanted = 500
# for d, l, f in zip(data, labels, file_paths):
#     f = ''.join(os.path.basename(f).split(".")[:-1])
#     label = labels_map[l]
...
for d in ['Anthracnose', 'bird eye spot', 'white spot']:
    for filename in os.listdir(os.path.join(directory, d)): 
        file = os.path.join(directory, d, filename)
        ...
        augment_dataset(img, d, 10, filename.replace(".jpg", ""))
